...
<div class="home-cards">
  <CardGrid stagger>
    ...
    <Card title="One more thread, big impact">
      ...
      It's simple: without workers, one expensive request hogs the main thread and everything else waits. Even a single worker thread unblocks that, and your cheap routes keep flowing.
...
<CardGrid>
  ...
  <Card title="Data path: payload buffers">
    ...
    - Each worker gets two `SharedArrayBuffer`; one for requests, one for returns.
...
- **No busy-waiting by default:** idle workers spin briefly, then park until there's work. The host dispatcher backs off too. Your CPU isn't burning cycles on nothing.
