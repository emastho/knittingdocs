...
export const run = getCode("validation/schema_validate/schema_knitting.ts");
export const bench = getCode("validation/schema_validate/bench_schema_validate.ts");
export const utils = getCode("validation/schema_validate/utils.ts");

This example parses JSON strings, validates them against a Zod schema, and
returns typed results on either the main thread or a Knitting worker pool. If your app already does `JSON.parse` + validation and you want to see what offloading looks like, start here.
...
- `schema_knitting.ts` -- runs parsing and validation in host and Knitting modes
...
:::note
These results show a speedup for batched validation on the measured setup.
Compare both paths on your own hardware and batch size.
:::
...
Schema validation can work well in a pool: each call is independent, inputs and
outputs are small in this example, and Zod's type checking and error formatting
use CPU time. If you're validating hundreds of payloads per second -- API gateway, webhook ingestion, form processing -- batching them through a pool can free your main thread without changing any validation logic.
