---
title: JWT revalidation
description: Verify JWTs and reissue them with Web Crypto, without an external JWT library.
hero:
  title: 'JWT validation + renewal'
sidebar:
  order: 11
---

import { Tabs, TabItem } from '@astrojs/starlight/components';
import { Code } from '@astrojs/starlight/components';
import { getCode } from '../../../../lib/code-snippets';
import RuntimeRunTabs from '../../../../components/RuntimeRunTabs.astro';

export const run = getCode("validation/jwt_revalidation/jwt_knitting.ts");
export const bench = getCode("validation/jwt_revalidation/bench_jwt_revalidation.ts");
export const utils = getCode("validation/jwt_revalidation/utils.ts");

This example verifies JWT signatures with HMAC SHA-256, checks expiry, and
optionally reissues tokens in a worker pool. It uses built-in Web Crypto APIs
without an external JWT package.

## How it works

The host builds a batch containing both valid and expired JWTs. Each job verifies the signature, checks the renewal window (`renewWindowSec`, `renewUntil`), and either returns the validated claims or issues a fresh token. The result comes back as a stringified JSON response -- keeping structured-clone overhead low.

Three files:

- `jwt_knitting.ts` -- runs revalidation in host and Knitting modes
- `utils.ts` -- token verification, renewal logic, task exports
- `bench_jwt_revalidation.ts` -- host-vs-worker benchmark with `mitata`

## Example request and response

Input to the task is a JSON string like this:

```json
{
  "token": "<jwt>",
  "nowSec": 1767225600,
  "ttlSec": 180,
  "renewWindowSec": 30
}
```

The task returns a JSON string. A successful non-renewed response looks like:

```json
{
  "ok": true,
  "renewed": false,
  "token": "<same-jwt>",
  "sub": "user_42",
  "sid": "session_42",
  "exp": 1767225750,
  "canRenew": true
}
```

If the token is near expiry but still renewable, the same shape comes back with `renewed: true`
and a replacement `token`.

## Run

:::info
<RuntimeRunTabs
  bunCommand="bun src/jwt_knitting.ts"
  denoCommand="deno run -A src/jwt_knitting.ts"
  nodeCommand="npx tsx src/jwt_knitting.ts"
/>
:::

Expected output:

```
-- host mode --
  verified: 900  renewed: 80  rejected: 20

-- knitting mode (2 threads) --
  verified: 900  renewed: 80  rejected: 20
```

## Optional benchmark

:::info
<RuntimeRunTabs
  bunCommand="bun src/bench_jwt_revalidation.ts"
  nodeCommand="npx tsx src/bench_jwt_revalidation.ts"
/>
:::

The benchmark compares verification, optional renewal, and `JSON.stringify`
through direct imports (`host`) with the same workload through worker calls (`knitting`). Batched dispatch keeps the comparison stable.

:::note
Both paths use the same logic. The only variable is whether execution happens on the main thread or in a worker.
:::

## Code

<Tabs>
  <TabItem label="jwt_knitting.ts">
    <Code code={run} lang="ts" title={"jwt_knitting.ts"} />
  </TabItem>
  <TabItem label="bench_jwt_revalidation.ts">
    <Code code={bench} lang="ts" title={"bench_jwt_revalidation.ts"} />
  </TabItem>
  <TabItem label="utils.ts">
    <Code code={utils} lang="ts" title={"utils.ts"} />
  </TabItem>
</Tabs>

## Why offload JWT work

HMAC verification and key derivation are CPU-bound. On a busy API server handling hundreds of authenticated requests per second, that crypto work competes with route handling on the main thread. Moving it to a pool keeps your event loop responsive -- especially under mixed traffic where some routes are cheap and others do substantial authentication work.
