...
Start with [Hono server routes](/examples/data_transforms/rendering_output/hono_server). It's the most realistic example -- a real HTTP server with SSR, JWT, and health check routes.
...
[PDF invoices](/examples/data_transforms/rendering_output/pdf_invoices) renders PDFs on workers, and covers how to return binary from a task without silently mangling it.
