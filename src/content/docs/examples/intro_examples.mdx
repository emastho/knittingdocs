...
Start with [Hono server routes](/examples/data_transforms/rendering_output/hono_server). It combines SSR, JWT, and health check routes in one HTTP server.
...
[LaTeX paper indexing](/examples/data_transforms/validation/latex_papers) parses real arXiv LaTeX sources on workers, and measures where a decompress-then-parse job should be split.

**"I need to generate documents"**
[PDF invoices](/examples/data_transforms/rendering_output/pdf_invoices) renders PDFs on workers, and shows how to preserve the binary type when returning data from a task.

**"I just want to convert some text"**
[Markdown to HTML](/examples/data_transforms/rendering_output/markdown_to_html) is the simplest rendering example -- Markdown in, compressed bytes out.
...
- [LaTeX paper indexing](/examples/data_transforms/validation/latex_papers) -- parse real arXiv sources into indexable prose
...
- [PDF invoices](/examples/data_transforms/rendering_output/pdf_invoices) -- generate PDF documents and return the bytes
