...
This guide covers browser setup, unsupported features, and the errors you may
encounter.
...
:::danger
Host your own copy of `/knitting.js`. This site serves the file with
`Access-Control-Allow-Origin: *` for the smoke test; it is not a CDN. It is
...
The page needs cross-origin isolation to use `SharedArrayBuffer`. Set these
headers:
...
Knitting requires shared memory, so the pool cannot start without cross-origin
isolation.
...
With this setup, the page loads `knitting.js` by URL, each task module
...
`knitting/browser` ships as a single self-contained file. The copy hosted here is
