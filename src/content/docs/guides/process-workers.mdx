...
The tasks and `call` API stay the same. The following sections explain how to
run the worker process inside a sandbox or container.
...
This example uses `permission: "unsafe"` because the container provides the
isolation boundary. The worker runs without extra runtime permission flags, so
restrict access through the container image and volume mounts. See
