...
are different. A pooled region has one owner and one release obligation; a
...
The consumer receives bytes, not a body it should recycle. When a descriptor is
...
uses `BufferReference` in the choose-one helper because it is likely large
enough for the materialize-then-move path to win. `BufferReference` is for
...
The host side has a limit too, and it is the sharper one. A borrowed argument
