...
description: "Every createPool option in one place: threads, balancers, work stealing, the inliner lane, payload buffers, permissions, debug namespaces, and shutdown."
sidebar:
  order: 2
---
...
When `unsafe.SharedArgs` is enabled, the returned pool also exposes
...
const input = pool.sharedArgBytes(256 * 1024);
input.set(source);
await pool.call.task(input);
```
...
[Shared memory](/guides/shared-memory/#shared-byte-arguments-and-returns) before
using this option. For borrowed worker returns, enable `unsafe.SharedBytes` and use
`sharedBytes()` from `knitting/unsafe`; see [Buffer reference](/guides/buffer-reference/)
...
See [Multi-threading](/guides/multi-threading/) for choosing a worker count and
configuring idle-worker timers in a server.
...
`SharedArrayBuffer`s, and `"fixed"` otherwise. If you request `"growable"` on an
...
How many abort-aware calls the pool can track at once, default `258`. It applies only
...
`processSharedMemory`. See [Process workers](/guides/process-workers) for
...
A wall-clock timeout on every task call. When it expires, Knitting shuts down
the whole pool, stopping even a worker stuck in a tight CPU loop.
...
Selects the region-claim discipline for work stealing. The default is
...
[Work stealing](/guides/work-stealing/) for the topology and limits.
...
On Node thread workers, requests the optional native `uv_async_t` completion
