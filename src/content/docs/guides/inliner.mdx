...
too -- useful when every task is pure computation and you want the host core to help
process it.
...
The inliner works well for **math and pure-compute workloads** that run on the host
without touching the network or filesystem:

- Number crunching, scoring, hashing, and matrix operations.
...
For compute workloads, higher values let the host process more work
...
At low concurrency, workers handle all calls. Once concurrency reaches the
threshold, the host also runs tasks.
...
the host thread. The toolkit still carries `now()`, which behaves normally and
...
const results = await Promise.all(
  chunks.map((chunk) => pool.call.scoreChunk(chunk)),
);
```
